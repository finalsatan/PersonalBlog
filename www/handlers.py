...
def index( *, page = '1' ):
    ...
    logging.info( 'Page: %s, BlogsNum: %s.' % ( page, len( blogs ) ) )
    return {
...
def get_posts( *, page = '1' ):
    page_index = get_page_index( page )
    num = yield from Post.findNumber( 'count(id)' )
    page = Page( num, page_index )
    if num == 0:
        posts = []
    else:
        posts = yield from Post.findAll( orderBy = 'created_at desc', limit = ( page.offset, page.limit ) )  
    logging.info( 'Page: %s, BlogsNum: %s.' % ( page, len( posts ) ) )
    return {
        '__template__' : 'posts.html',
        'page' : page,
        'posts' : posts
    }
